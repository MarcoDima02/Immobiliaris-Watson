...
export default function AdminInfoContainer({
    ...
    async function getDettagliImmobile(id: number) {
        try {
            const response = await fetch(
                `http://localhost:8080/api/admin/dashboard/richieste/${id}/dettagli`,
                {
                    method: 'GET',
                    headers: {
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch(
                `http://localhost:8080/api/admin/dashboard/utenti/${selectedUser.idUtente}`,
                {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
        ...
        <>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-10 px-8">
                {listToRender?.map((item) => (
                    <div
                        key={item.id}
                        className="bg-white p-4 w-full rounded-xl shadow-xl text-black flex flex-col mb-5"
                    >
                        {type === 'utenti' && (
                            <>
                                ...
                                <div className="flex flex-col w-full">
                                    <div className="w-full flex justify-evenly mb-4">
                                        ...
                                        <div className="w-full">
                                            ...
                                            <p className="text-zinc-600">
                                                {item.dataRichiesta?.slice(0, 10).replace(/-/g, '/')}{' '}
                                                alle {item.dataRichiesta?.slice(11, 16)}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col my-3 text-zinc-600">
                                        ...
                                        <p>{item.superficieMq} metri quadri</p>
                            ...
                            <div className="flex flex-col gap-3 mb-5">
                                <div className="flex flex-row flex-wrap w-full">
                                    ...
                                    <Button
                                        ...
                                        onClick={() => handleClick(item.idImmobile)}
                                    >
                                        Visualizza immobile
                                    </Button>
